...
export function PspWarning({ type }: PspWarningProps) {
    ...
    return (
        <div className="bg-surface-dark border border-amber-500/20 rounded-2xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-6 relative group overflow-hidden">
            <div className="absolute inset-0 bg-amber-500/5 pointer-events-none" />
            <div className="absolute -top-24 -left-24 w-48 h-48 bg-amber-500/10 rounded-full blur-[80px] group-hover:bg-amber-500/20 transition-all duration-700 pointer-events-none" />

            <div className="flex items-start gap-4 relative z-10">
                <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-full flex-shrink-0">
            ...
            <Link
                href={settingsPath}
                className="relative z-10 flex items-center gap-2 px-6 py-2.5 bg-amber-500 text-black font-bold rounded-lg hover:bg-amber-400 transition-all duration-200 group/btn whitespace-nowrap shadow-glow hover:shadow-glow-lg"
            >
                {t("cta")}
