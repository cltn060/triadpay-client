...
export default function TenantAffiliatePage({
    ...
    useEffect(() => {
        if (!isClerkLoaded || store === undefined) return; // Wait for both to load

        // Seller clicked an affiliate link — redirect them away
        if (user && isSeller) {
        ...
        if (user && isAffiliate && joinState === "idle") {
            setJoinState("joining");
            requestMembership({ storeSlug: slug })
                .then(() => {
                    setJoinState("done");
    ...
    }, [isClerkLoaded, user, store, isAffiliate, isSeller, joinState, slug, requestMembership, router]);

    // --- Loading States ---

    // Global loading: waiting for Clerk + store data
    if (!isClerkLoaded || store === undefined) {
        return (
    ...
    // Affiliate is logged in — show the "connecting" state
    if (user && isAffiliate && joinState !== "error") {
        return (
                ...
                <div className="flex flex-col items-center gap-6 text-center max-w-md px-6">
                    <div className="relative">
                        <div className="w-20 h-20 rounded-full bg-[#0df20d]/10 border-2 border-[#0df20d]/30 flex items-center justify-center animate-pulse">
                            <span className="material-icons text-[#0df20d] text-3xl">handshake</span>
                        </div>
                    </div>
                    <div>
                        ...
                        <p className="text-gray-500 text-sm leading-relaxed">
                            Setting up your partnership request. You&apos;ll be redirected to your portal in a moment...
                        </p>
                    </div>
    ...
    // --- Standard Landing Page (User is NOT logged in, or is a seller being redirected) ---

    // Preserve refSeller in the sign-up URL if present
    const searchParams = typeof window !== "undefined" ? new URLSearchParams(window.location.search) : null;
    const refSeller = searchParams?.get("refSeller") ?? "";
    const signUpUrl = `/sign-up/affiliate?store=${store.slug}${refSeller ? `&refSeller=${refSeller}` : ""}`;
    ...
    return (
        <div className="relative flex flex-col min-h-screen w-full overflow-x-hidden bg-[#050505] font-sans text-white" style={{
            ...
            <main className="flex-grow flex flex-col items-center justify-center px-6 py-20 text-center relative z-10">
                <div className="max-w-4xl mx-auto space-y-8">
                    ...
                    <h1 className="text-5xl md:text-7xl font-black text-white leading-[1.1] tracking-tighter">
                        Partner with <br />
                        <span className="text-[#0df20d] italic">{store.name}</span>
                    </h1>
            ...
            {/* Error State (inline, only if magic join failed) */}
            {joinState === "error" && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-red-500/10 border border-red-500/20 rounded-xl px-6 py-4 flex items-center gap-3 z-50 max-w-md">
                    ...
                    <div>
                        <p className="text-red-400 text-sm font-bold">Oops! Something went wrong</p>
                        <p className="text-red-400/70 text-xs">We could not create your partnership request. Please try signing up below.</p>
                    </div>
                </div>
            )}
        </div>
